...
class VideoSwiper {
    ...
    updateSwiper() {
        ...
        // Определяем data-index для каждого слота
        ...
        this.videos.forEach((container) => {
            // Получаем data-index текущего контейнера из DOM
            const containerDataIndex = parseInt(container.getAttribute('data-index'));

            // Удаляем все классы слотов перед назначением нового
            container.classList.remove('is-center-slot', 'is-left-slot', 'is-right-slot');

            const videoEl = container.querySelector('video');

            // Назначаем класс в зависимости от того, какому data-index соответствует контейнер
            if (containerDataIndex === activeDataIndexToFind) {
                container.classList.add('is-center-slot');
                if (videoEl) {
                    if (document.visibilityState === 'visible') {
                        videoEl.play().catch(e => console.warn("Video play failed for active item:", e));
                    } else {
                        videoEl.pause(); // Пауза, если вкладка не активна
                    }
                }
            ...
            // Поскольку у нас всегда 3 видео, каждое получит один из трех классов слотов.
        });
        ...
        // Блокируем повторную смену слайда на время анимации
        setTimeout(() => {
            this.isTransitioning = false;
        }, 600); // Должно соответствовать времени transition в CSS
    }
}
